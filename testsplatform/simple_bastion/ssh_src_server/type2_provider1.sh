...
function generate_tech1_data_files
{
    ...
    do
        ...
        do
            gzip_data_file "$path/dataresult_127${fset}_20_${date}${hs}00_${date}${hs}20.csv"
            gzip_data_file "$path/dataresult_127${fset}_60_${date}${hs}00_${date}${he}00.csv"
        done
        tar_data_files "$path/../TECH1_${date}${hs}00.tar.gz" "$path"
    done
    rm -rf "$path"
}
...
function generate_tech2_data_files
{
    ...
    do
        ...
        for fset in ${functionsetids[@]}
        do
            gzip_data_file "$path/dataresult_${fset}_20_${date}${hs}00_${date}${hs}20.csv"
            gzip_data_file "$path/dataresult_${fset}_60_${date}${hs}00_${date}${he}00.csv"
        done
        tar_data_files "$path/../TECH2_${date}${hs}00.tar.gz" "$path"
    done
    rm -rf "$path"
}
...
function generate_tech3_data_files
{
    ...
    do
        ...
        for fset in ${functionsetids[@]}
        do
            gzip_data_file "$path/A${date}.${hs}00+0100-${he}00+0100_GQSHB.xml"
        done
        tar_data_files "$path/../TECH3_${date}${hs}00.tar.gz" "$path"
    done
    rm -rf "$path"
}
